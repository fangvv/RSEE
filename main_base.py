...
def validate(val_loader, model, criterion, epoch, logger, exp_full_path, tf_writer=None):
    ...
    with torch.no_grad():
        for i, input_tuple in enumerate(val_loader):
            ...
            if use_ada_framework:
                if args.irte_final: # TODO cat fake data
                    _b, _t, _reso = r.shape
                    compl_array = np.zeros((_b, args.num_segments - _t, _reso))
                    compl_array.fill(-1)
                    r_align = np.hstack((r.cpu().numpy(), compl_array))
                    r_list.append(r_align)
                else:
            ...
            if i % args.print_freq == 0:
                ...
                if use_ada_framework:
                    ...
                    if args.irte_final:
                        print_output += f'\texit [{exit_log}]'
                    print_output += extra_each_loss_str(each_terms)

                print(print_output)

    if args.irte_final:
        print(info_all_data.numpy())
    ...
    if use_ada_framework:
        usage_str, gflops = get_policy_usage_str(r_list, model.module.reso_dim, isTraining=False, tf_writer=tf_writer, epoch=epoch)
        print(usage_str)
    ...
    if tf_writer is not None:
        ...
        tf_writer.add_scalar('GFLOPs', gflops, epoch) if use_ada_framework else ...

    return mAP, mmAP, top1.avg, usage_str if use_ada_framework else None, gflops, info_all_data


def train_irte(train_loader, model, criterion, optimizer, epoch):
    # TODO every reso loss
    batch_time, data_time, losses, top1, top5 = get_average_meters(5)
    tau = 1
    ...
    for i, input_tuple in enumerate(train_loader):
        ...
        input_var_list = [torch.autograd.Variable(input_item).to(device) for input_item in input_tuple[:-1]]
        
        num_sizes = len(input_var_list)
        if args.online_train:
            outputs, exit_log = model.module.forward_bk_ol(input=input_var_list, tau=tau)
        elif args.irte_final:
            outputs = model.module.forward_irte(input=input_var_list, tau=tau)
        else:
            outputs = model.module.forward(input=input_var_list, tau=tau)

        for size_idx in range(num_sizes):
            loss_t = 0
            if args.online_train:
                for t in range(args.num_segments):
                    loss_t += get_criterion_loss(
                        criterion, outputs[size_idx][t], target_var
                    )
                loss += loss_t / args.num_segments
            else:
                loss += get_criterion_loss(criterion, outputs[size_idx], target_var)
        if args.online_train:
            loss /= num_sizes
        # measure accuracy and record loss
        # prec1, prec5 = accuracy(output.data, target[:, 0], topk=(1, 5))
        losses.update(loss.item(), input_var_list[0].size(0))
        # top1.update(prec1.item(), input.size(0))
        # top5.update(prec5.item(), input.size(0))
...
def validate_irte(val_loader, model, criterion, epoch, logger, exp_full_path, tf_writer=None):
    batch_time, losses = get_average_meters(2)
    top1 = [AverageMeter() for _ in range(len(args.reso_list))]
    top5 = [AverageMeter() for _ in range(len(args.reso_list))]
    num_sizes = len(args.reso_list)
    tau = 1
    # TODO(yue)
    all_results = []
    all_targets = []
    ...
    with torch.no_grad():
        for i, input_tuple in enumerate(val_loader):
            ...
            target = input_tuple[-1].to(device)
            if args.online_train:
                outputs, _exit_ = model.module.forward_bk_ol(input=input_var_list, tau=tau)
            elif args.irte_final:
                outputs = model.module.forward_irte(input=input_var_list)
            else:
                outputs = model.module.forward(input=input_var_list)
            ...
            for size in range(num_sizes):
                if args.online_train:
                    ...
                    for t in range(args.num_segments):
                        loss_t += get_criterion_loss(
                            criterion, outputs[size][t], target
                        )
                    loss += loss_t / args.num_segments
                else:
                    loss += get_criterion_loss(criterion, outputs[size], target)
            ...
            for size in range(num_sizes):
                if args.online_train:
                    prec1, prec5 = accuracy(outputs[size][-1], target[:, 0], topk=(1, 5))
                else:
                    prec1, prec5 = accuracy(outputs[size], target[:, 0], topk=(1, 5))
                top1[size].update(prec1.item(), input.size(0))
                top5[size].update(prec5.item(), input.size(0))
            ...
            if i % args.print_freq == 0 and i != 0:
                print_output = (f'Test: [{i:03d}/{len(val_loader):03d}] '
                                f'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                                f'Loss {losses.val:.4f} ({losses.avg:.4f})\n')
                for size in range(num_sizes):
                    print_output += (
                        f'reso{args.reso_list[size]}: '
                        f'Prec@1 {top1[size].val:.3f} ({top1[size].avg:.3f}) '
                        f'Prec@5 {top5[size].val:.3f} ({top5[size].avg:.3f})\n' 
                    )
                print(print_output)

    print(f'Testing: Loss {losses.avg:.5f}\n')
    for size in range(num_sizes):
        print(f'reso{args.reso_list[size]}: Prec@1 {top1[size].avg:.3f} Prec@5 {top5[size].avg:.3f}')

    gflops = 0

    if tf_writer is not None:
        tf_writer.add_scalar('loss/test', losses.avg, epoch)
        tf_writer.add_scalar('acc/test_top1', top1[0].avg, epoch)
        tf_writer.add_scalar('acc/test_top5', top5[0].avg, epoch)

    return -1, -1, top1[0].avg, None, gflops
...
def load_resnet50_weights_imagenet(model, weights):
    new_weights = {}
    model_dict = model.state_dict()
    for k, v in weights.items():
        k_split = k.split('.')
        if ('conv1' in k_split or 'conv2' in k_split or 'conv3' in k_split) or \
            ('downsample' in k_split and 'weight' in k_split and k_split[-2] == '0'):
            k_split.insert(-1, 'module')
            k_split = ['base_model'] + k_split
            new_k = '.'.join(k_split)
            new_weights[new_k] = v
        # if ('bn1' in k_split or 'bn2' in k_split or 'bn3' in k_split):
        #     for i in range(len(args.reso_list)):
        #         spec_k_split = k_split.copy()
        #         spec_k_split.insert(-1, 'bn' + str(i))
        #         spec_k_split = ['base_model'] + spec_k_split
        #         new_k = '.'.join(spec_k_split)
        #         new_weights[new_k] = v
...
def show_exit_log(exit_log):
    """
        exit_log: T * [B, 2]
    """
    exit_log_tensor = torch.stack(exit_log, dim=1).detach().cpu().numpy() # [B, T, 2]
    last_one = exit_log_tensor[-1] # [T, 2]
    return reverse_onehot(last_one)

def save_info_data(path, a):
    file_name = ospj(path, "info.npy")
    a_numpy = a.numpy()
    np.save(file_name, a_numpy)
